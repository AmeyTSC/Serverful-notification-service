...
import { SMS_Template } from 'src/schemas/sms_template.schema';
import { SmsErrorLog } from 'src/schemas/smserror_logs.schema';
import { SmsHitLog } from 'src/schemas/smshit_log.schema';
import { SmsTemplateService } from 'src/sms_template/sms_template.service';
import { smsSent } from 'src/util/sms_send';
...
async smsSqs(templateAttributes: any): Promise<any> {
  ...
  private async createSmsHitLog(reqObject: any, smsResponse: any): Promise<SmsHitLog> {
    const smsHitLog = new this.SmsHitLogModel({
  ...
  private async createSmsErrorLog(orderId: string, errorMsg: string): Promise<SmsErrorLog> {
    const smsErrorLog = new this.SmsErrorLogModel({
      order_id: orderId,
      error: errorMsg,
